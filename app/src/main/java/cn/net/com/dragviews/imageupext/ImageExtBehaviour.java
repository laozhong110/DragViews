...
public class ImageExtBehaviour<V extends View> extends CoordinatorLayout.Behavior<View> {
    ...
    public ImageExtBehaviour() {
        super();
    }

    public ImageExtBehaviour(Context context, AttributeSet attrs) {
        super(context, attrs);
...
//        }
        ...
        mAnimateRunable = new ImageExtBehaviour.AnimateRunable(context);
        mPictureRecovyAnimateRunable = new ImageExtBehaviour.PictureRecovyAnimateRunable(context);
    }
    ...
    public void onNestedPreScroll(CoordinatorLayout coordinatorLayout, View child, View target, int dx, int dy, int[] consumed) {
        ...
        if (target instanceof RecyclerView) {
            ...
            if (mImageViweSrcHeight == -1) {
                if (child0View instanceof ViewGroup && ((ViewGroup) child0View).getChildCount() > 0) {
                    mImageViweSrcHeight = ((ViewGroup) child0View).getChildAt(0).getHeight();
                    int iweSrcHeight = ((ViewGroup) child0View).getChildAt(1).getHeight();
                    int x = 0;
                    x++;
                } else {
            ...
            if (bCanScroll && dy > 0 && child0Top + mImageViweSrcHeight >= 0) {//整体往上
                if (child0Top + mImageViweSrcHeight - dy > 0) {
                    int offset = consumed[1] = dy;
                    if (-1 != mImageViweSrcHeight && child0View instanceof ViewGroup && ((ViewGroup) child0View).getChildAt(0).getHeight() > mImageViweSrcHeight) {//向上压缩
                        if (((ViewGroup) child0View).getChildAt(0).getHeight() - dy >= mImageViweSrcHeight) {
                            child0View.getLayoutParams().height -= dy;
                            coordinatorLayout.requestLayout();
                        } else if (((ViewGroup) child0View).getChildAt(0).getHeight() - dy < mImageViweSrcHeight) {
                            child0View.getLayoutParams().height -= (((ViewGroup) child0View).getChildAt(0).getHeight() - mImageViweSrcHeight);
                            coordinatorLayout.requestLayout();
                        }
                    } else if (-1 != mImageViweSrcHeight && !(child0View instanceof ViewGroup) && child0View.getLayoutParams().height > mImageViweSrcHeight) {//向上压缩
                        //保留以前的代码,来直接兼容ImageView
                ...
                } else if (child0Top + mImageViweSrcHeight - dy <= 0) {
                    int offset = consumed[1] = (child0Top + mImageViweSrcHeight);
                ...
                if (-1 != mImageViweSrcHeight && !(child0View instanceof ViewGroup) && child0View.getLayoutParams().height > mImageViweSrcHeight) {//向上压缩
                    if (child0View.getLayoutParams().height - dy >= mImageViweSrcHeight) {
                        child0View.getLayoutParams().height -= dy;
                        coordinatorLayout.requestLayout();
                    } else if (child0View.getLayoutParams().height - dy < mImageViweSrcHeight) {
                ...
                } else if (-1 != mImageViweSrcHeight && (child0View instanceof ViewGroup) && ((ViewGroup) child0View).getChildAt(0).getHeight() > mImageViweSrcHeight) {//向上压缩
                    if (((ViewGroup) child0View).getChildAt(0).getHeight() - dy >= mImageViweSrcHeight) {
                        child0View.getLayoutParams().height -= dy;
                        coordinatorLayout.requestLayout();
                    } else if (((ViewGroup) child0View).getChildAt(0).getHeight() - dy < mImageViweSrcHeight) {
    ...
    public void onStopNestedScroll(CoordinatorLayout coordinatorLayout, View child, View target) {
        ...
        View child0View = coordinatorLayout.getChildAt(0);
        // int h = ((ViewGroup) child0View).getChildAt(0).getHeight();
        //    int child0Top = child0View.getTop();
        //   int child0Height = child0View.getHeight();
        if (-1 != mImageViweSrcHeight && !(child0View instanceof ViewGroup) && child0View.getLayoutParams().height > mImageViweSrcHeight) {
            mPictureRecovyAnimateRunable.startScroll(coordinatorLayout, 0, child0View.getHeight(), 0, mImageViweSrcHeight - child0View.getHeight());
        } else if (-1 != mImageViweSrcHeight && (child0View instanceof ViewGroup) && ((ViewGroup) child0View).getChildAt(0).getHeight() > mImageViweSrcHeight) {
            int startY = child0View.getHeight();
            int dy = mImageViweSrcHeight - ((ViewGroup) child0View).getChildAt(0).getHeight();
            Log.d("RecovyAnimateRunable", "run() scrolling startY:" + String.valueOf(startY) + " dy:" + String.valueOf(dy));
            mPictureRecovyAnimateRunable.startScroll(coordinatorLayout, 0, startY, 0, dy);
        }
    }
    ...
    public boolean onNestedPreFling(CoordinatorLayout coordinatorLayout, View child, final View target, final float velocityX, final float velocityY) {
        ...
        if ((-1 != mImageViweSrcHeight && !(child0View instanceof ViewGroup) && child0View.getLayoutParams().height > mImageViweSrcHeight)
                || (-1 != mImageViweSrcHeight && (child0View instanceof ViewGroup) && ((ViewGroup) child0View).getChildAt(0).getHeight() > mImageViweSrcHeight)) {
            fullCanScroll = false;
        }
        if (fullCanScroll) {
    ...
    ImageExtBehaviour.AnimateRunable mAnimateRunable;
    ...
    class AnimateRunable implements Runnable {
        ...
        public void run() {
            if (mScroller.computeScrollOffset()) {
                ...
                if (currY > 0) {//往上
                    if (child0Top + mImageViweSrcHeight > 0) {
                        ...
                        if (childCount > 0 && lastCompletelyVisibleItemPosition >= childCount - 1) {//处理RecyclerView 的item不足够多的情况,比如没有或只有一两个,即内部不可以滑动
                            ...
                            int curOffset = currY - mPrePosition;
                            if (child0Top + mImageViweSrcHeight - curOffset <= 0) {
                                curOffset = child0Top + mImageViweSrcHeight;
                            }
    ...
    ImageExtBehaviour.PictureRecovyAnimateRunable mPictureRecovyAnimateRunable;
    ...
    public boolean onMeasureChild(CoordinatorLayout parent, View child, int parentWidthMeasureSpec, int widthUsed, int parentHeightMeasureSpec, int heightUsed) {
        ...
        if (parent.getChildCount() < 2 && child instanceof RecyclerView) {
            throw new IllegalStateException("the child~s count must is > 2 and the second childer must is RecyclerView Type");
        }
        ...
        int topHeight = 0;
        View child0View = parent.getChildAt(0);
        if (child0View instanceof ViewGroup) {
            topHeight = ((ViewGroup) child0View).getChildAt(1).getLayoutParams().height;
        }

        //   int child0Height = parent.getChildAt(0).getMeasuredHeight();

        int height = parentHeight - topHeight/* - child0Height*/;
